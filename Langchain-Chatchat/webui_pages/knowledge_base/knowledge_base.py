...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    ...
    if selected_kb == "新建知识库":
        ...
        if cols[1].button(
            ...
            print("************************************")
            print("************************************")
            print("文件"+os.path.abspath(__file__)+"第"+str(sys._getframe().f_lineno)+"行，前端添加重命名知识库按钮。")
            print("************************************")
            print("************************************")
            with st.form("重命名知识库"):
                new_kb_name = st.text_input(
                    f"知识库新名字(当前知识库名字为{kb})",
                    placeholder="新知识库名称，不支持中文命名",
                    key="new_kb_name",
                )
            #     submit_rename_kb = st.form_submit_button("确认",use_container_width=True)
            # print("&&&&&&&&&&&&&&&&&&&&&&&&&")
            # print(submit_rename_kb)
            # print("&&&&&&&&&&&&&&&&&&&&&&&&&")
            # if submit_rename_kb:
                # if not new_kb_name or not new_kb_name.strip():
                #     st.error(f"知识库新名称不能为空！")
                # elif new_kb_name in kb_list:
                #     st.error(f"名为 {new_kb_name} 的知识库已经存在！")
                # else:
                ret = api.rename_knowledge_base(
                    kb_name=kb,
                    new_kb_name="test",
                )
                st.toast(ret.get("msg", " "))
                st.session_state["selected_kb_name"] = kb
                st.session_state["selected_kb_new_name"] = "test"
                st.rerun()

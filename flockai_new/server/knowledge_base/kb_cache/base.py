...
class EmbeddingsPool(CachePool):
    def load_embeddings(self, model: str = None, device: str = None) -> Embeddings:
        ...
        logging.info(f"模型地址为：{get_model_path(model)}")
        ...
        if not self.get(key):
            ...
            with item.acquire(msg="初始化"):
                ...
                if model == "text-embedding-ada-002":  # openai text-embedding-ada-002
                    ...
                    # model,cache_folder=get_model_path(model).replace(model,"")
                    embeddings = SentenceTransformer(get_model_path(model)).to(device)
